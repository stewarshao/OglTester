...
{
	static int eclapse = 1;

	if (0 == eclapse % 2)
		prog.setUniform("LightPosition", view * vec4(1.0f, 0.0f, 0.0f, 0.0f) );
	else
		prog.setUniform("LightPosition", view * vec4(1.0f, 0.0f, 0.0f, 1.0f) );
...
{
    if( ! prog.compileShaderFromFile("../shader/cookbook/directional.vs",GLSLShader::VERTEX) )
    {
        printf("Vertex shader failed to compile!\n%s",
               prog.log().c_str());
        exit(1);
    }
    if( ! prog.compileShaderFromFile("../shader/cookbook/directional.fs",GLSLShader::FRAGMENT))
    {
