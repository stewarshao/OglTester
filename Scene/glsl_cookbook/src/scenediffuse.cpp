...
void SceneDiffuse::setMatrices()
{
	...
	// vec3(mv[0]) represent the first column vector of mv
...
{
    if( ! prog.compileShaderFromFile("../shader/cookbook/diffuse.vert",GLSLShader::VERTEX) )
    {
        printf("Vertex shader failed to compile!\n%s",
               prog.log().c_str());
        exit(1);
    }
    if( ! prog.compileShaderFromFile("../shader/cookbook/diffuse.frag",GLSLShader::FRAGMENT))
    {
