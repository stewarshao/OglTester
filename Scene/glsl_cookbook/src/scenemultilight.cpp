...
{
    if( ! prog.compileShaderFromFile("../shader/cookbook/multilight.vert",GLSLShader::VERTEX) )
    {
        printf("Vertex shader failed to compile!\n%s",
               prog.log().c_str());
        exit(1);
    }
    if( ! prog.compileShaderFromFile("../shader/cookbook/multilight.frag",GLSLShader::FRAGMENT))
    {
